...
import { Handler, Callupdate, GroupUpdate } from './king/kirkuu/index.js';
...
async function downloadSessionData() {
  ...
  const _0x331b8a = _0xccedb8.SESSION_ID.split("Kirikuumd$")[0x1];
...
async function start() {
  try {
    ...
    console.log("KIRIKUU_MD is running on v" + _0x2f6d2f.join('.') + ", isLatest: " + _0x23f0a4);
    const _0x76bf4 = makeWASocket({
      ...
      'getMessage': async _0x53ca5a => {
        ...
        return {
          'conversation': "BEST WHATSAPP BOT MADE BY Raheem-cm"
        };
      }
    });
    _0x76bf4.ev.on("connection.update", _0x4eb449 => {
      ...
      if (_0x237ed1 === "close") {
        ...
        if (initialConnection) {
          console.log(_0x4b0fc4.green("KIRIKUU_MD MD CONNECTED SUCCESSFULLY ✅"));
          _0x76bf4.sendMessage(_0x76bf4.user.id, {
            'text': "╭─────────────━┈⊷\n│ *kirikuu ɪs ᴄᴏɴɴᴇᴄᴛᴇᴅ*\n╰─────────────━┈⊷\n\n╭─────────────━┈⊷\n│🤖 ʙᴏᴛ ɴᴀᴍᴇ: *KIRIKUU_ᴍᴅ*\n│👨‍💻 ᴏᴡɴᴇʀ : *Raheem-cm *\n╰─────────────━┈⊷\n\n*Message Me on whatsapp 😈*\n_https://wa.me/255763111390_"
          });
...
async function init() {
  if (_0x5687e2.existsSync(credsPath)) {
    ...
    if (_0x17d9d4) {
      console.log("KIRIKUU_MD IS RUNNING...⏳");
      await start();
    } else {
...
app.get('/', (_0x1ecf21, _0x282bcc) => {
  _0x282bcc.send("KIRIKUU_MD IS CONNECTED SUCCESSFULLY ✅");
});
app.listen(PORT, () => {
  console.log("KIRIKUU_MD daily users " + PORT);
});
